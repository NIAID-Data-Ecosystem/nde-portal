...
export const SamplesDisplay = ({ sample }: SamplesDisplayProps) => {
  ...
  return (
    <Flex flexDirection='column' gap={8}>
      ...
      {/* {sampleQuantity?.list?.data && sampleQuantity.list.data.length > 0 && (
        <Table
          ariaLabel={sampleQuantity.list.caption}
          caption={sampleQuantity.list.caption}
          data={sampleQuantity.list.data || []}
          tableContainerProps={{ overflowY: 'auto', maxHeight: '400px' }}
          getTableRowProps={(_, idx) => ({
            bg: idx % 2 === 0 ? 'white' : 'page.alt',
          })}
          getCells={props => {
