        ...
        datasets: [
            {
                ...
                otherNames,
                // backgroundColor: function (one: Object) {
                //     let label = Object.keys(dataCatalogResponse)[one['dataIndex']]
                //     let index = Object.keys(dataCatalogResponse).indexOf(label)
                //     if (label === 'Omics Discovery Index') {
                //         return 'blue'
                //     }
                // },
    ...
    if (responseData['date'].length > 0) {
        ...
        dateInfo.forEach(obj => {
            ...
            if (!moment(obj['term']).isAfter()) {
                const rounded = moment(obj['term']).startOf('quarter').format('YYYY-MM-DD');
                // catalogObj[rounded]
                //     ? (catalogObj[rounded] += obj['count'])
                //     : (catalogObj[rounded] = obj['count']);
                if (catalogObj[rounded]) {
                    catalogObj[rounded] += obj['count']
                } else if (!catalogObj[rounded]) {
                    catalogObj[rounded] = obj['count']
                }
                // catalogObj[rounded] = obj['count']
            }
